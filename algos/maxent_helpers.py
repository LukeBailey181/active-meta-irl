...
def local_action_probabilities(p_transition, terminal, reward):
    ...
    for _ in tqdm(range(250)):
        za = np.array([er * p[a].dot(zs) for a in range(n_actions)]).T      # TODO this line is causing NaNs
        if np.isnan(za).any():
            breakpoint()
        if np.isinf(za).any():
            breakpoint()
    ...
    output = np.divide(za, zs[:, None], where=zs[:, None]!=0)
    return output 
...
def irl(p_transition, features, terminal, trajectories, optim, init, eps=1e-4, eps_esvf=1e-5):
    ...
    print("Feature expectations done")
    p_initial = initial_probabilities_from_trajectories(n_states, trajectories)
    print("Initial probabilities from trajectories done")
    ...
    while delta > eps:
        ...
        # TODO this is bugges rn
        e_svf = compute_expected_svf(p_transition, p_initial, terminal, reward, eps_esvf)   # This is a very slow step
